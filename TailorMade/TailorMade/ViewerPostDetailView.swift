...
struct ViewerInstagramPostView: View {
    var post: Post
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            NavigationLink(destination: OfferingView(post: post)) {
                Text("Offer")
                    .font(.headline)
                    .padding()
    ...
    private func updateBookmarkInFirestore() {
        ...
        if isBookmarked {
            // Add post to user's saved posts collection
            db.collection("users").document(userId).collection("saved_posts").document(postId).setData(["postId": postId, "imageUrl": post.imageUrl]) { error in
                if let error = error {
    ...
    private func addComment() {
        ...
        let commentsRef = db.collection("posts").document(postId).collection("comments").document()
